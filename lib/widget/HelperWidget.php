 ...
 * <li>Опишите поля, которые будут переданы связи в EntityManager. Поля описываются в настройке "MULTIPLE_FIELDS" виджета.
 * По умолчанию множественный виджет использует поля ID, ENTITY_ID, VALUE</li>
 ...
 * <ul>
 * <li>в интерфейсе раздела опишите поле, укажите множественный виджет и установите параметр MULTIPLE => true</li>
 * <li>создайте таблицу и модель к ней, в них должны хранится значения множественного виджета.
 * Таблица должна иметь обязательные поля, которые описаны в настройке MULTIPLE_FIELDS.
 * По умолчанию множественные виджеты требуют наличие полей: ID (INT), ENTITY_ID (INT), VALUE, но набор полей может отличаться.
 * Смотрите комментарии к виджетам. Если у вас нет возможности создать в модели требуемые поля, вы можете указать их алиасы.
 * Например если в вашей моделе есть поля ID, MODEL_ID, LINK, а виджет требует ID, ENTITY_ID, VALUE, то в параметре поля MULTIPLE_FIELDS
 * укажите алиасы полей (передавая этот параметр, вы перезаписываете настройку, поэтому обязательно передавайте все названия полей,
 * которые требует виджет, даже те, для которых вы не пропишете алиас).
 * Например так:
 * 'RELATED_LINKS' => [
 ...
 * ],
 * </li>
 * <li>укажите связь в главной модели с созданной моделью во втором пункте. Ниже указан рекомендуемый формат описания связи:
...
abstract class HelperWidget
{
	...
	protected $settings = [
		...
		'MULTIPLE_FIELDS' => ['ID', 'VALUE', 'ENTITY_ID']
	];
