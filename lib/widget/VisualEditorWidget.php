...
class VisualEditorWidget extends TextAreaWidget
{
    ...
    protected function genEditHTML()
    {
        if (\CModule::IncludeModule("fileman")) {
            ...
            $className = OrmClasses::getShortName($this->modelObject);
            $id = isset($this->modelObject->{$this->modelObject->pk()}) ? $this->modelObject->{$this->modelObject->pk()} : false;
            ...
            $this->modelObject->loadFields($codeType);
            
            CFileMan::AddHTMLEditorFrame(
                $bxCode,
                $this->modelObject->{$this->code},
                $bxCodeType,
                $this->modelObject->$codeType,
                array(
            ...
            $contentType = $this->modelObject->$codeType;
    ...
    public function processEditAction()
    {
        ...
        switch ($currentView) {
            case HelperWidget::EDIT_HELPER:
                ...
                $this->modelObject->{$this->code} = $_REQUEST[$bxCode];
                $this->modelObject->$codeType = $_REQUEST[$bxCodeType];
    ...
    {
        return $this->modelObject->{$this->code};
    }
